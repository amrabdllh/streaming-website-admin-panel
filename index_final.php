...
// Dynamic views system - update views randomly on each page load
$stmt = $pdo->prepare("SELECT id FROM videos");
$stmt->execute();
$videoIds = $stmt->fetchAll(PDO::FETCH_COLUMN);

foreach ($videoIds as $videoId) {
    // Random increment between 1-10 views per page load
    $randomIncrement = rand(1, 10);
    $stmt = $pdo->prepare("UPDATE videos SET views = views + ? WHERE id = ?");
    $stmt->execute([$randomIncrement, $videoId]);
}

// Ensure all videos have at least 50,000 views
$stmt = $pdo->prepare("UPDATE videos SET views = GREATEST(views, 50000 + FLOOR(RAND() * 50000))");
$stmt->execute();
...
if (isset($_GET['video_id'])) {
    ...
    // Update view count with random increment
    $randomIncrement = rand(5, 25);
    $stmt = $pdo->prepare("UPDATE videos SET views = views + ? WHERE id = ?");
    $stmt->execute([$randomIncrement, $videoId]);
